...
void Camera::moveCameraToCenter(float x, float y, int time)
{
	targetMovePosition.x = ((x*zoomLevel)*-1) + screen_width / 2;
	targetMovePosition.y = ((y*zoomLevel)*-1) + screen_height / 2;
	moveToTargetTime = time;
	moveToTargetSpeed_x = 0;
	moveToTargetSpeed_y = 0;
