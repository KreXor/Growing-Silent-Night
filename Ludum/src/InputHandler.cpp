...
void InputHandler::Init()
{
	...
	keyOpenMenu.keycode = SDLK_m;
	keyReturn.keycode = SDLK_RETURN;
...
void InputHandler::CheckPlayerMovement(SDL_Event* event, GameEngine* game)
{
	...
	if (event->type == SDL_KEYDOWN)
	{
		...
		if (event->key.keysym.sym == keyOpenMenu.keycode){
			keyOpenMenu.pressed = true;
		}

		if (event->key.keysym.sym == keyReturn.keycode){
			keyReturn.pressed = true;
		}
	...
	if (event->type == SDL_KEYUP)
	{
		...
		if (event->key.keysym.sym == KeyEscape.keycode){
			KeyEscape.pressed = false;
		}

		if (event->key.keysym.sym == keyOpenMenu.keycode){
			keyOpenMenu.pressed = false;
		}

		if (event->key.keysym.sym == keyReturn.keycode){
			keyReturn.pressed = false;
		}
	}
